...
func (m *ListBlockStreamsResponse_Result) CloneVT() *ListBlockStreamsResponse_Result {
	...
	if rhs := m.Streams; rhs != nil {
		tmpContainer := make([]*BlockStreamInfo, len(rhs))
		for k, v := range rhs {
...
func (m *ListBlockStreamsResponse_Result) MarshalToSizedBufferVTStrict(dAtA []byte) (int, error) {
	...
	if len(m.Streams) > 0 {
		for iNdEx := len(m.Streams) - 1; iNdEx >= 0; iNdEx-- {
			size, err := m.Streams[iNdEx].MarshalToSizedBufferVTStrict(dAtA[:i])
			if err != nil {
				return 0, err
			}
			i -= size
...
func (m *ListBlockStreamsResponse_Result) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 2:
			...
			m.Streams = append(m.Streams, &BlockStreamInfo{})
...
func (m *ListBlockStreamsResponse_Result) UnmarshalVTUnsafe(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 2:
			...
			m.Streams = append(m.Streams, &BlockStreamInfo{})
			if err := m.Streams[len(m.Streams)-1].UnmarshalVTUnsafe(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		default:
